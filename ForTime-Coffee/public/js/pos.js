...
function initDiscountActions() {
    ...
    const selectCode = document.getElementById('discount-code'); // Ô chọn mã

    // 1. XỬ LÝ KHI NGƯỜI DÙNG THAY ĐỔI LỰA CHỌN (MỚI THÊM)
    if (selectCode && btnApply) {
        selectCode.addEventListener('change', function() {
            // Khi chọn mã mới, lập tức đổi nút Hủy -> nút Áp dụng
            btnApply.innerHTML = 'Áp dụng';
            btnApply.className = 'btn btn-outline-primary';
            
            // Xóa các class trang trí (màu xanh/nền xám) của trạng thái "đã áp dụng"
            selectCode.classList.remove('bg-light', 'text-success', 'fw-bold');
        });
    }

    // 2. XỬ LÝ KHI BẤM NÚT (GIỮ NGUYÊN LOGIC CŨ)
    if (btnApply) {
        btnApply.addEventListener('click', function() {
            if (!currentTableId) return;
            
            // TRƯỜNG HỢP 1: NÚT ĐỎ (ĐANG HỦY MÃ)
            // Chỉ chạy khi nút đang ở trạng thái 'btn-danger' VÀ người dùng chưa đổi sang mã khác
            if (this.classList.contains('btn-danger')) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        callDiscountApi(''); // Gửi mã rỗng để hủy
                    }
                });
            } 
            // TRƯỜNG HỢP 2: NÚT XANH (ÁP DỤNG MÃ MỚI / THAY ĐỔI MÃ)
            else {
                const code = selectCode.value.trim();
                
                // Nếu chọn "Chọn mã ưu đãi" (value rỗng) -> Gọi API để hủy mã hiện tại (nếu có)
                if (!code) { 
                    callDiscountApi(''); 
                    return; 
                }

                // Gọi API áp dụng mã mới
